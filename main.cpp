...
int main() {
    int numOfCases, numOfTests, numOfRep, temp;
    ...
    for(int xx=0;xx<numOfTests;xx++){//dla każdego testu
        ...
        numOfRep=1;
        std::cin>>numOfCases;
        auto *arrayForClients = new std::string[numOfCases];
//        std::string arrayForClients[numOfCases];
        std::fflush(stdin);
        for(int i=0;i<numOfCases;i++){//pobieranie numerow
            std::getline(std::cin, arrayForClients[i]);
        }

        mergeSort(arrayForClients, 0, numOfCases - 1);//sortowanie listy
